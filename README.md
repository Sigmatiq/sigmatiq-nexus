...
- `PARTICIPANT_FLOW_CONTEXT` also treats repeated high-premium institutional clusters with a clear premium-side bias as structurally usable, even when quote width leaves many individual labels low-confidence. That keeps main-symbol windows from failing closed on retail-noise alone while still degrading windows where bid-side premium materially overwhelms confirmed ask-side flow.
